...
THREE.STLLoader.prototype.parseBinary = function ( data ) {
	...
	var dataOffset = 84;// header + 4 bytes for int32
	var faceLength = 12 * 4 + 2;// 12 floats each float is 4 bytes; 3 for (nx,ny,nz) + 9 for vertices + 2 for attribute
	...
	for ( var face = 0; face < faces; face ++ ) {
		nx = reader.getFloat32( start , true );
		ny = reader.getFloat32( start + 4, true );
		nz = reader.getFloat32( start + 8, true );
		normal = new THREE.Vector3(nx,ny,nz);
		start = start + 12;
		for ( var i = 0; i < 3; i ++ ) {
			x = reader.getFloat32( start, true );
			y = reader.getFloat32( start + 4, true );
			z = reader.getFloat32( start + 8, true );
			geometry.vertices.push( new THREE.Vector3(x,y,z));
			start = start + 12;
		}
		length = geometry.vertices.length;
		geometry.faces.push( new THREE.Face3( length - 3, length - 2, length - 1, normal ) );
		attribute = reader.getUint16( start, true );// no need to store
		start = start + 2;// 2 bytes for attribute
	}
